...
class GraphSubst:
    ...
    def __call__(self, expr: relay.Expr) -> relay.Expr:
        ...
        The substituted graph is returned. Mutated parameters are stored in the object.
    ...
    def next_param_name(self) -> str:
        self.update_next_idx()
        return self.fmt_param_name(self.next_idx)

    def update_next_idx(self):
        while self.params.__contains__(self.fmt_param_name(self.next_idx)):
            self.next_idx += 1

    @staticmethod
    def fmt_param_name(idx: int):
...
class _SubstMutator(relay.ExprMutator):
    ...
    def visit_call(self, call: relay.Call) -> relay.Expr:
        ...
        # Match call node with pattern
        if not self.match(self.pattern, call):
            # Not matched, return the original node, but with new arguments
            return relay.Call(call.op, new_args, attrs=call.attrs)

        # Perform substitution
        return self.subst(call)

    # noinspection PyTypeChecker
    @staticmethod
    def match(pat: relay.Expr, expr: relay.Expr) -> bool:
        ...
        if isinstance(pat, relay.Call):
            return _SubstMutator.match_call(pat, expr)
    ...
    @staticmethod
    def match_call(pat: relay.Call, expr: relay.Call) -> bool:
        # Match operator
        ...
        for pat_arg, expr_arg in zip(pat.args, expr.args):
            ...
            if isinstance(pat_arg, relay.Var):
                continue
            if not _SubstMutator.match(pat_arg, expr_arg):
                return False

        return True

    @staticmethod
    def match_tuple_getitem(pat: relay.TupleGetItem, expr: relay.TupleGetItem) -> bool:
        return pat.index == expr.index and \
               _SubstMutator.match(pat.tuple_value, expr.tuple_value)
